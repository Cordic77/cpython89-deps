...
#endif
    {
    //MiSt: no 'mode_t' type - pass directly!
    #ifndef _WIN32
		const mode_t mode = S_IRUSR | S_IWUSR;
		pair->dest_fd = open(pair->dest_name, flags, mode);
    #else
		pair->dest_fd = open(pair->dest_name, flags, S_IRUSR | S_IWUSR);
    #endif
    }}
...
io_read(file_pair *pair, io_buf *buf_union, size_t size)
{
...

  {
	...
	while (left > 0) {
		const ssize_t amount = read(pair->src_fd, buf, (unsigned int)left);
...
io_write_buf(file_pair *pair, const uint8_t *buf, size_t size)
{
	...
	while (size > 0) {
		const ssize_t amount = write(pair->dest_fd, buf, (unsigned int)size);
		if (amount == -1) {
			if (errno == EINTR) {
				if (user_abort)
					return true;
...
io_write(file_pair *pair, const io_buf *buf, size_t size)
{
	...
	if (pair->dest_try_sparse) {
		...
		if (size == IO_BUFFER_SIZE) {
			if (is_sparse(buf)) {
//W			pair->dest_pending_sparse += size;
				pair->dest_pending_sparse = (off_t)(pair->dest_pending_sparse + size);
				return false;
			}
		} else if (size == 0) {
			return false;
