...
//			+ lz_options->after_size) / 2 + (UINT32_C(1) << 19);
    reserve = (uint32_t)(reserve + (lz_options->before_size + lz_options->match_len_max + lz_options->after_size) / 2 + (UINT32_C(1) << 19));
...
lzma_lz_encoder_memusage(const lzma_lz_options *lz_options)
{
	...
	*/
	...
	lzma_mf mf = {
		NULL,   //uint8_t *buffer;
		0,      //uint32_t size;
		0,      //uint32_t keep_size_before;
		0,      //uint32_t keep_size_after;
		0,      //uint32_t offset;
		0,      //uint32_t read_pos;
		0,      //uint32_t read_ahead;
		0,      //uint32_t read_limit;
		0,      //uint32_t write_pos;
		0,      //uint32_t pending;
		NULL,   //uint32_t (*find)(lzma_mf *mf, lzma_match *matches);
		NULL,   //void (*skip)(lzma_mf *mf, uint32_t num);
		NULL,   //uint32_t *hash;
		NULL,   //uint32_t *son;
		NULL,   //uint32_t cyclic_pos;
		0,      //uint32_t cyclic_size; // Must be dictionary size + 1.
		0,      //uint32_t hash_mask;
		0,      //uint32_t depth;
		0,      //uint32_t nice_len;
		0,      //uint32_t match_len_max;
		{0},    //lzma_action action;
		0,      //uint32_t hash_size_sum;
		0,      //uint32_t sons_count;
	};
	*/
	lzma_mf mf = {0};
