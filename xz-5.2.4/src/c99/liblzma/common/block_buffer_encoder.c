...
lzma_block_buffer_bound(size_t uncompressed_size)
{
  ...
  /*MiSt: warning C4244: 'return': conversion from 'uint64_t' to '::size_t', possible loss of data*/
	return (size_t)ret;
}
		...
		uint8_t *out, size_t *out_pos, size_t out_size)
{
	...
	*out_pos += block->header_size;
    ...
    /*MiSt: warning C4267: '=': conversion from 'size_t' to 'uint8_t', possible loss of data*/
		out[(*out_pos)++] = (uint8_t)((copy_size - 1) >> 8);
		out[(*out_pos)++] = (uint8_t)((copy_size - 1) & 0xFF);
		...
		uint8_t *out, size_t *out_pos, size_t out_size)
{
	...
	*out_pos += block->header_size;
    ...
    /*MiSt: warning C4244: '=': conversion from 'lzma_vli' to '::size_t', possible loss of data*/
		out_size = (size_t)(*out_pos + block->compressed_size);
