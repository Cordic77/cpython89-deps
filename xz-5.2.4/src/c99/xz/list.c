...
parse_indexes(xz_file_info *xfi, file_pair *pair)
{
    ...
    /*MiSt: warning C4244: '-=': conversion from 'lzma_vli' to 'off_t', possible loss of data*/
	//pos -= index_size;
    pos = (off_t)(pos - index_size);
		...
		do {
			...
			// Index size.
      /*MiSt: warning C4244: '=': conversion from 'lzma_vli' to '::size_t', possible loss of data*/
    ...
    pos = (off_t)(pos - (footer_flags.backward_size + LZMA_STREAM_HEADER_SIZE));
    ...
    pos = (off_t)(pos - lzma_index_total_size (this_index));
...
#	error IO_BUFFER_SIZE < LZMA_BLOCK_HEADER_SIZE_MAX
  ...
  /*MiSt: warning C4244: 'initializing': conversion from 'const lzma_vli' to 'const uint32_t', possible loss of data*/
	const uint32_t size = (uint32_t)(my_min(iter->block.total_size
				- lzma_check_size(iter->stream.flags->check),
			LZMA_BLOCK_HEADER_SIZE_MAX));
	io_buf buf;
  /*MiSt: warning C4244: 'function': conversion from 'const lzma_vli' to 'off_t', possible loss of data*/
	if (io_pread(pair, &buf, size, (off_t)iter->block.compressed_file_offset))
		return true;
...
parse_check_value(file_pair *pair, const lzma_index_iter *iter)
{
  ...
  /*MiSt: warning C4244: 'initializing': conversion from 'const lzma_vli' to 'const off_t', possible loss of data*/
	const off_t offset = (off_t)(iter->block.compressed_file_offset
